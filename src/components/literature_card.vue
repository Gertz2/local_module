...
const props = defineProps({
  ...
  dateOpen: {
    type: String,
    default: "понедельник, 1 сентября 2025, 00:00",
  },
});

const topicTitle = ref(props.topic);
const literatureText = ref(props.literature);
const dateOpenText = ref(props.dateOpen);

watch(props, () => {
  topicTitle.value = props.topic;
  literatureText.value = props.literature;
  dateOpenText.value = props.dateOpen;
});
...
const saveChanges = () => {
  if (
    ...
    !literatureText.value.trim() ||
    !dateOpenText.value.trim()
  ) {
